...
function StatusIcon({ status }: { status: LocalMessage['status'] }) {
  if (status === 'seen')      return <CheckCheck size={14} style={{ color: '#53bdeb' }} />;
  if (status === 'delivered') return <CheckCheck size={14} style={{ color: 'rgb(var(--text-muted))' }} />;
  if (status === 'sent')      return <Check      size={14} style={{ color: 'rgb(var(--text-muted))' }} />;
  if (status === 'failed')    return <span style={{ color: '#ff3b3b', fontSize: 12, fontWeight: 700 }}>!</span>;
  return <Check size={14} style={{ color: 'rgb(var(--text-muted))' }} />;
}
      ...
      <div style={{ maxWidth: '65%', display: 'flex', flexDirection: 'column', alignItems: isMine ? 'flex-end' : 'flex-start', gap: 2 }}>
        ...
        <div style={{ position: 'relative', display: 'flex', alignItems: 'flex-end', gap: 4 }}>
          ...
          <div style={{
            ...
            padding: hasMedia ? '3px 3px 24px' : '7px 12px 22px',
